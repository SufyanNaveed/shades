...
class Productcategory extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $sum_all_product_stock_price = 0;
        if($data['cat']){
            foreach($data['cat'] as $cat){
                $sum_all_product_stock_price = $sum_all_product_stock_price + $cat['salessum'];
            }
        }
        $data['sum_stock_price'] = $sum_all_product_stock_price;
    ...
    public function view()
    {
        ...
        $products_list = $this->products->get_datatables($data['id'],'',$data['sub']);
        $sum_all_product_stock_price = 0;
        if($products_list){
            foreach($products_list as $product){
                $sum_all_product_stock_price = $sum_all_product_stock_price + ($product->qty * $product->product_price);
            }
        }
        $data['sum_stock_price'] = $sum_all_product_stock_price;
