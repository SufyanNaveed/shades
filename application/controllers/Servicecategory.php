...
class Servicecategory extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('categories_model', 'services_cat');
    ...
    {
        $data['cat'] = $this->services_cat->service_category_stock();
        $head['title'] = "Service Categories";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('services/category', $data);
    ...
    public function warehouse()
    {
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('services/warehouse', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function view()
    {
        ...
        $data['cat'] = $this->services_cat->category_list();
        // $data['cat'] = $this->products_cat->category_sub_stock($data['id']);
        $head['title'] = "View Service Category";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('services/category_view', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function viewwarehouse()
    {
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('services/warehouse_view', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function add()
    {
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('services/category_add', $data);
        $this->load->view('fixed/footer');
    }
    ...
    {
        $cat_name = $this->input->post('service_catname', true);
        $cat_desc = $this->input->post('service_catdesc', true);
        $cat_type = $this->input->post('cat_type', true);
        $cat_rel = $this->input->post('cat_rel', true);
        if ($cat_name) {
            $this->services_cat->addServiceCat($cat_name, $cat_desc, $cat_type, $cat_rel);
        }
    }
    ...
    public function delete_i()
    {
        if ($this->aauth->premission(11)) {
            ...
            if ($id) {
                ...
                $query = $this->db->query("DELETE geopos_movers FROM geopos_movers LEFT JOIN geopos_services ON  geopos_movers.rid1=geopos_services.pid LEFT JOIN geopos_service_cat ON  geopos_services.pcat=geopos_service_cat.id WHERE geopos_service_cat.id='$id' AND  geopos_movers.d_type='1'");

                $this->db->delete('geopos_services', array('pcat' => $id));
                $this->db->delete('geopos_service_cat', array('id' => $id));
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('Service Category with services')));
            } else {
...
//view for edit
    public function edit()
    {
        ...
        $this->db->select('*');
        $this->db->from('geopos_service_cat');
        $this->db->where('id', $catid);
        $query = $this->db->get();
        $data['servicecat'] = $query->row_array();
        $data['cat'] = $this->services_cat->category_list();

        $head['title'] = "Edit Service Category";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('services/service-cat-edit', $data);
        $this->load->view('fixed/footer');
    ...
    public function editcat()
    {
        ...
        $service_cat_desc = $this->input->post('service_cat_desc');
        $cat_type = $this->input->post('cat_type', true);
        $cat_rel = $this->input->post('cat_rel', true);
        $old_cat_type = $this->input->post('old_cat_type', true);
        if ($cid) {
            $this->services_cat->edit($cid, $service_cat_name, $service_cat_desc, $cat_type, $cat_rel, $old_cat_type);
        }
    }
